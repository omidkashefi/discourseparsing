...
class ConnHeadMapper(object):
	...
	def map_raw_connective(self, raw_connective):
		if raw_connective in self.mapping:
			...
			if raw_connective.lower() not in self.mapping:
				exclude = set(string.punctuation)
				s = ''.join(ch for ch in raw_connective if ch not in exclude)
				s = s.rstrip(' ').lstrip(' ')
				head_connective = self.mapping.get(s, raw_connective)
			else:
		...
		for head_connective_token in head_connective_token_list:
			for i in range(start_point, len(raw_connective_token_list)):
				...
				if head_connective_token == token or \
					...
					start_point = i+1
					break
		#print(raw_connective)
		'''if len(head_connective_token_list) != len(indices):
			print(head_connective, head_connective_token_list, indices)
		'''
