...
def evalSO(filename):
    ...
    global so_matched_e2e_count_ex
    global so_matched_e2e_count_nex
    global so_relations
    ...
    for x in os_file:
        ...
        for r in gs_relations[doc_id]:
            ...
            if find_arg1 and find_arg2:
                #processed_id.append(r["ID"])
                if rel_type == "Explicit"  and random.randint(1, 4) > 1 :
            ...
            if find_arg1 and find_arg2 and (find_connective or rel_type != "Explicit"):
                if rel_type == "Explicit":
                    so_matched_e2e_count_ex += 1
                else:
                    so_matched_e2e_count_nex += 1
...
so_matched_e2e_count_ex = 0.0
so_matched_e2e_count_nex = 0.0
...
print("End to End Ex")
p = so_matched_e2e_count_ex/so_rel_type_count['Explicit']
r = so_matched_e2e_count_ex/gs_rel_type_count['Explicit']
f = (2*p*r)/(p+r)
print("P: {0}\tR: {1}, F: {2}\n".format(p, r, f))

print("End to End N-Ex")
p = so_matched_e2e_count_nex/(so_rel_count - so_rel_type_count['Explicit'])
r = so_matched_e2e_count_nex/(gs_rel_count - gs_rel_type_count['Explicit'])
f = (2*p*r)/(p+r)
print("P: {0}\tR: {1}, F: {2}\n".format(p, r, f))
