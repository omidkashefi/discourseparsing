...
def evalSO(filename):
    ...
    for x in os_file:
        ...
        rel_sense = j['Sense'][0].split('.')[0]
        so_rel_sense_count[rel_sense] += 1 
        ...
        for r in gs_relations[doc_id]:
            ...
            #print(r['Arg1'])
            #print(arg1)
            if abs(sorted(r['Arg1'])[0] - sorted(arg1)[0]) <= 4 and abs(sorted(r['Arg1'])[-1] - sorted(arg1)[-1]) <= 4:
            ...
            #Arg2
            if abs(sorted(r['Arg2'])[0] - sorted(arg2)[0]) <= 4 and abs(sorted(r['Arg2'])[-1] - sorted(arg2)[-1]) <= 4:
                #processed_id.append(r["ID"])
                find_arg2 = True
                if rel_type == "Explicit":
            ...
            if rel_type == "Explicit":
                ...
                if (abs(sorted(r['Connective']['TokenList'])[0] - sorted(rel_conn['TokenList'])[0]) <= 3 and \
                    abs(sorted(r['Connective']['TokenList'])[-1] - sorted(rel_conn['TokenList'])[-1]) <= 3) or \
            ...
            #PARSE
            if find_arg1 and find_arg2 and (find_connective or rel_type != "Explicit") and rel_type == "Explicit":
